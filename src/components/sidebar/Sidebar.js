...
export default function Sidebar () {
  ...
  return (
    <div className={`bg-white h-full sm:relative rounded-t-xl flex-col absolute w-full z-10 sm:visible ${!open && 'invisible'}`}>
      <button
        onClick={() => setOpen((prev) => !prev)}
        className="visible relative sm:invisible mt-8 ml-10 ">
      ...
      <div className='flex flex-col xl:ml-16 sm:ml-4 gap-8 text-gray-500 sm:text-xl text-2xl items-center sm:items-start '>
        <Link
          className={`flex gap-1 hover:text-gray-900 items-center  ${selectedOption === 'Inicio' ? 'text-gray-900' : ''}`}
          prefetch={false}
          href="/">
          <HomeIcon />
          Inicio
        ...
        <Link
          className={`flex gap-1 hover:text-gray-900 items-center  ${selectedOption === 'Pesquisar' ? 'text-gray-900' : ''}`}
          prefetch={false}
          href="/search">
          <SearchIcon />
          Pesquisar
